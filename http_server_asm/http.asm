...
read_req_path:
    ...
    mov rsi, req_path   ; store in req_buffer
    mov rdx, 128        ; read only 1 line (<=128 bytes)
    syscall
    ...
    mov rsi, req_path
    mov rdx, 3          ; compare 3 chars
    call strncmp
    cmp rax, 0          ; read again if not match
    jne read_req_path

    mov eax, [req_html] ; add html in front of path
    mov [req_path], eax

    mov rcx, 127        ; max 127 bytes
    mov al, 32          ; space
    mov rdi, req_path   ; skip "GET"
...
;-------------------------------------------
; open_req_file()
;   Open the requested file, save the fd in file_fd.
...
;-------------------------------------------
; make_default_index()
;   If the path ends with '/', append index name.
